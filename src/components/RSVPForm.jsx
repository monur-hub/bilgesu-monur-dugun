...
export function RSVPForm() {
  const [form, setForm] = useState({ name: '', attending: true, guests: 2, events: [EVENT_OPTIONS[0]], note: '' });
  ...
  const toggleEvent = (opt) => setForm((f) => ({
    ...f,
    events: f.events.includes(opt) ? f.events.filter((ev) => ev !== opt) : [...f.events, opt],
  }));
  ...
  const submit = async () => {
    ...
    const eventsToSend = form.attending ? (form.events.length ? form.events : [EVENT_OPTIONS[0]]) : [EVENT_OPTIONS[0]];
    eventsToSend.forEach((ev) => params.append(GOOGLE_FORM_ENTRIES.event, ev));
          ...
          <div style={{ background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(8px)', border: '1px solid rgba(140,115,85,0.18)', borderRadius: 22, padding: 36, boxShadow: '0 20px 50px rgba(90,70,40,0.1)' }}>
            ...
            {form.attending && (
              <>
                ...
                <label style={labelStyle}>Hangi Etkinliğe (birden fazla seçebilirsiniz)</label>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 22 }}>
                  {EVENT_OPTIONS.map((opt) => {
                    const checked = form.events.includes(opt);
                    return (
                      <label
                        key={opt}
                        style={{
                          display: 'flex', alignItems: 'center', gap: 10, padding: '11px 14px', borderRadius: 10,
                          border: checked ? '1px solid #4a5d3a' : '1px solid rgba(140,115,85,0.3)',
                          background: checked ? 'rgba(74,93,58,0.08)' : '#fff', cursor: 'pointer',
                          fontFamily: "'Cormorant Garamond',serif", fontSize: 16, color: '#5c4a3a', transition: 'all 0.2s',
                        }}
                      >
                        <input
                          type="checkbox"
                          ...
                          style={{ width: 18, height: 18, accentColor: '#4a5d3a', flexShrink: 0 }}
                        />
                        {opt}
                      </label>
                    );
